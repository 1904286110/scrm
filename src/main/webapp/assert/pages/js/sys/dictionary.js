...
layui.use(['layer', 'util', 'treeTable','iconPicker'], function () {
        ...
        var insTb = treeTable.render({
            ...
            cols: [
                ...
                {field: 'dictInfo', title: '字典的描述'},
                {field: 'dictKey', title: '字典KEY'},
                {field: 'createDate', title: '创建时间', templet: function (d) {
                        return util.toDateString(d.createDate);
                    }, width: 180
                },
                {align: 'center', toolbar: '#dictionaryBtnTpl', title: '操作', width: 200}
            ],
            style: 'margin-top:0;',
        ...
        treeTable.on('tool(list_table)', function (obj) {
            ...
            switch (event) {
			case 'dic_child'://进新增子资源
				//打开通用的弹出窗口
				openBaseLayer('dictionary/child/'+rowId,'子资源新增').done(function(){
					// 让form表单渲染一下。 form_add_edit = <form lay-filter="form_add_edit">
					form.render(null, 'form_add_edit');
				});
				break;
			case 'dic_edit':
				openBaseLayer('dictionary/form','资源修改').done(function(){
					$.ajax({
						url:'dictionary/'+rowId,
     ...
     // 监听提交动作  submit(but_submit) = <button class="layui-btn" lay-submit lay-filter="but_submit">
    	form.on('submit(but_dictionary_submit)', function(data) {
...
/*        function initActionForm(index,obj){
        ...
        //自定义 校验
        form.verify({
        	checkdictInfo:function(value,item){
        		return checkUnique(value, item,'dictionary/checkname',{"parentKey":$('#parentKey').val()})
        	}
        });
/*        //绑定 资源类型 select 
